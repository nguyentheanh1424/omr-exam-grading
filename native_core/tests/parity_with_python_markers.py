...
OUTPUT_DIR = REPO_ROOT / "results" / "parity_python_markers"
...
def build_python_pipeline() -> tuple[WarpEngine, OMRProcessor]:
    ...
    warp_engine = WarpEngine(str(REPO_ROOT / TEMPLATE_LAYOUT_FILE), str(TEMPLATE_IMAGE))
...
def run_python_pipeline(
    ...
    return {"score": int(result["score"]), "answers": [int(v) for v in result["answers"]]}


def python_detected_markers(img_path: Path) -> list[tuple[int, float, float]]:
    image = read_image(img_path)
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    return [(int(det.id), float(det.center[0]), float(det.center[1])) for det in detect_tags(gray)]
...
def run_native_pipeline(
    ...
    img = read_image(image_path)
    detected_markers = python_detected_markers(image_path)
    result = client.run(
        img,
        config,
...
def compare_answers(py_answers: list[int], native_answers: list[int]) -> dict[str, Any]:
    ...
    for index, (py_value, native_value) in enumerate(zip(py_answers, native_answers)):
        if py_value != native_value:
            mismatches.append({"question": index, "python": py_value, "native": native_value})
    return {"match": len(mismatches) == 0, "mismatch_count": len(mismatches), "mismatches": mismatches}
...
def main() -> None:
    ...
    for image_path in image_paths:
        ...
        print(
            ...
            f"markers={native_result['detected_marker_count']}",
        )
